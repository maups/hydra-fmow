...
def prepare_data(params):
    ...
    if (params.train_cnn):
       ...
       if params.path[len(params.path)-1] == '/':
          walkDirs = [os.path.basename(params.path[:-1])]
       else:
          walkDirs = [os.path.basename(params.path)]
        ...
        for root, dirs, files in tqdm(os.walk(os.path.join(params.directories['dataset'], currDir))):
            ...
            for file in files:
                if file.endswith('_rgb.json'): #skip _msrgb images
                    taskA = partial(_process_file, file, slashes, root, isTrain, outDir, paramsDict, False, 1.0, 1)
                    taskB = partial(_process_file, file, slashes, root, isTrain, outDir, paramsDict, False, 0.3, 2)
                    futuresA.append(executor.submit(taskA))
                    futuresB.append(executor.submit(taskB))
                    futuresC.append(executor.submit(taskA))
                elif file.endswith('_msrgb.json'): #skip _rgb images
                    taskC = partial(_process_file, file, slashes, root, isTrain, outDir, paramsDict, True, 1.0, 1)
                    futuresC.append(executor.submit(taskC))

    record_files (params, futuresA, 1)
    record_files (params, futuresB, 2)
    record_files (params, futuresC, 3)
    # Shutdown the executor and free resources
    executor.shutdown()

def record_files (params, futures, option):
    ...
    if (option == 1):
       if (params.train_cnn):
         json.dump(trainingData, open(params.files['training_struct1'], 'w'))
         json.dump(metadataStats, open(params.files['dataset_stats1'], 'w'))
       else:
         json.dump(testData, open(params.files['test_struct1'], 'w'))
    elif (option == 2):
       if (params.train_cnn):
         json.dump(trainingData, open(params.files['training_struct2'], 'w'))
         json.dump(metadataStats, open(params.files['dataset_stats2'], 'w'))
       else:
         json.dump(testData, open(params.files['test_struct2'], 'w'))
    elif (option == 3):
       if (params.train_cnn):
         json.dump(trainingData, open(params.files['training_struct3'], 'w'))
         json.dump(metadataStats, open(params.files['dataset_stats3'], 'w'))
       else:
         json.dump(testData, open(params.files['test_struct3'], 'w'))

def _process_file(file, slashes, root, isTrain, outDir, params, is_msrgb, zoom, option):
    """
    ...
    for bb in jsonData['bounding_boxes']:
        ...
        if (option == 1):   
           featuresPath = os.path.join(currOut, baseName + 'a_features.json')
           imgPath = os.path.join(currOut, baseName + 'a.jpg')
        elif (option == 2):   
           featuresPath = os.path.join(currOut, baseName + 'b_features.json')
           imgPath = os.path.join(currOut, baseName + 'b.jpg')
        ...
        widthBuffer = int((box[2] * (contextMultWidth * zoom)) / 2.0)
        heightBuffer = int((box[3] * (contextMultHeight * zoom)) / 2.0)
        ...
        if (box[2] <= 96 or box[3] <= 96) and (is_msrgb):
            continue
